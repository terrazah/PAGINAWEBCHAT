-- Chat Konekto: corrección definitiva de bandeja unificada por contacto.
-- Ejecutar después de 20260817_message_threads.sql.

-- Es necesario eliminar la firma anterior antes de redefinirla para evitar
-- el error PostgreSQL 42P13 cuando la función cambia su declaración.
DROP FUNCTION IF EXISTS public.compartir_foto_segura(text, uuid, uuid, text);
  ...
  RETURN jsonb_build_object(
    ...
    'messages', coalesce((
      WITH base_messages AS (
        SELECT
          m.id, m.remitente_id, m.receptor_id, m.tipo_mensaje,
          m.contenido, m.imagen_url, m.foto_id, m.creado_en, m.leido_en,
          g.tipo_regalo,
      ...
      contact_messages AS (
        SELECT
          base.*,
          count(*) FILTER (WHERE base.receptor_id = profile_id AND base.leido_en IS NULL)
            OVER (PARTITION BY base.contacto_id) AS mensajes_no_leidos,
          row_number() OVER (
            PARTITION BY base.contacto_id
            ORDER BY base.creado_en DESC, base.id DESC
          ) AS contact_row
        FROM base_messages base
      )
      ...
      FROM contact_messages latest
      WHERE latest.contact_row = 1
    ), '[]'::jsonb)
  );
END;
$$;
...
BEGIN
  ...
  SELECT CASE WHEN m.remitente_id = viewer_id THEN m.receptor_id ELSE m.remitente_id END
    INTO other_id
    FROM public.mensajes_privados m
  ...
  UPDATE public.mensajes_privados AS pm
   ...
   WHERE pm.receptor_id = viewer_id
...
BEGIN
  ...
  -- p_conversacion_id se conserva por compatibilidad con clientes anteriores.
  -- La bandeja y el chat se unifican siempre por la pareja de contactos.
...
BEGIN
  ...
  INSERT INTO public.mensajes_privados (
    remitente_id, receptor_id, tipo_mensaje, contenido, foto_id, imagen_url
  )
  SELECT sender_id, p_receptor_id, 'foto', message_text, p_foto_id, f.url
...
SECURITY DEFINER SET search_path = public AS $$
BEGIN
  RETURN public.compartir_foto_segura(p_session_token, p_receptor_id, p_foto_id, p_mensaje);
END;
$$;
...
BEGIN
  ...
  DELETE FROM public.mensajes_privados AS pm
   WHERE (pm.remitente_id = viewer_id AND pm.receptor_id = other_id)
      OR (pm.remitente_id = other_id AND pm.receptor_id = viewer_id);
